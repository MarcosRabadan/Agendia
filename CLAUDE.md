...
> 📌 **Última actualización mayor: 2026-05-23 (2ª sesión).** Cerrado **#52 (soft delete + audit fields)** y, tras una **auditoría multi-agente** del código, varias correcciones de bugs/seguridad y refactors — todo mergeado a master: **#125** (fix cross-tenant en update de Employee), **#127** (soft-delete: NPE notificaciones, negocio borrado no reservable, índice IsDeleted), **#128** (`RepositoryBase<T>` + `AuditableEntity` base), **#129** (robustez de arranque/config + códigos de error de dominio en el validador + idioma de excepciones), **#133** (integridad de citas: doble-reserva por query filters + mass-assignment en UpdateAppointment + reset de recordatorio), **#135** (eliminar código muerto). **134/134 tests verdes.**
>
> ⚠️ **El usuario cambió la POLÍTICA DE TRABAJO:** ahora las **correcciones de bugs y refactors** las hago en **automático de principio a fin (incluido mergear a master con `gh pr merge --admin`)**; las **features nuevas** sí pasan por PR para que el humano la revise. Lee **"Política de trabajo y autonomía"** ANTES de nada. **Backlog: 3 issues abiertas** (#51-push, #55, #58).
...
- ✅ **134/134 tests verdes** (1 API.Tests placeholder + 87 unit + 46 integration).
...
│   ├── Persistence/           ← AuditableSaveChangesInterceptor (audit fields + Delete→soft delete) (#52)
...
├── MRC.Agendia.Tests.Unit/       ← 87 tests con xUnit + NSubstitute + EF InMemory
│   ├── TestDoubles/              ← FakeCurrentUserContext
│   ├── Domain/                   ← AppointmentStatusExtensionsTests (OccupiesCapacity)
│   └── Infrastructure/           ← Authorization/ResourceAuthorizationServiceTests, Services/ScheduleResolverTests, Notifications/NotificationServiceTests, Auditing/AuditLoggerTests, Persistence/AuditableSaveChangesInterceptorTests, Repositories/AppointmentRepositoryTests (query filters x soft-delete)
└── MRC.Agendia.Tests.Integration/← 46 tests con xUnit + WebApplicationFactory<Program> + InMemory
    ├── Infrastructure/           ← CustomWebApplicationFactory (con AuditableSaveChangesInterceptor) + FakeEmailSender + RequireConfirmedEmailWebApplicationFactory
    ├── Auth/                     ← AuthFlow, OwnerRegistration, PasswordReset, EmailConfirmation, RequireConfirmedEmail, LogoutAll, AuditLogEndpoint
...
# Tests (134/134 verdes a fecha de hoy)
dotnet test
...
5. **Tests:** `dotnet test` SIN regresiones (134/134 actual). Tras crear ficheros nuevos, `dotnet format` (el `.editorconfig` exige CRLF; los ficheros nuevos suelen salir con LF).
...
## Estado del backlog (2026-05-23, 2ª sesión) — 3 issues abiertas

> Cerradas en esta 2ª sesión (todas mergeadas): **#52** (soft delete + audit fields) y, de la auditoría: **#125** (cross-tenant Employee), **#127** (soft-delete behavior), **#128** (RepositoryBase + AuditableEntity), **#129** (robustez + error codes), **#133** (integridad de citas), **#135** (código muerto).
...
1. **Re-auditar el código con agentes** (la rutina que pidió el usuario): reviewers por área + verificadores; corregir en automático bugs/refactors (con su issue + auto-merge). Antes de reportar, revisar las "decisiones de auditoría ya aceptadas" (en "Política de trabajo y autonomía") para no re-levantarlas.
2. **Antes de producción real**: verificar el **envío SMTP real** (`SmtpEmailSender`) contra un relay (Mailtrap/SES). La lógica está hecha y testeada con un fake, pero el envío real no se ha probado contra un servidor.
3. **Push notifications (#51)**: feature nueva → **implementar y dejar en PR para revisión humana** (no auto-merge). Requiere decidir Firebase (FCM) + persistir device tokens (migración) — necesita decisión de cloud, hoy aparcada.
4. **NO atacar #55 (caching) ni #58 (global query filter)** sin métricas / sin incidente.
5. Deuda conocida sin issue: **tests unitarios de CRUD handlers** (Business/Client/Employee/Service/Appointment) y del `AppointmentSchedulingValidator`.

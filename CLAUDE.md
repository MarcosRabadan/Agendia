...
> 📌 **Última actualización: 2026-08-12 (9ª sesión).** ⚠️ **CAMBIO ARQUITECTÓNICO YA EN MASTER: Agendia es ahora un microservicio de Harmony (identidad externa).** Toda la auth de **usuario** se movió a Harmony. Agendia **ya no**: registra usuarios, hace login/logout/refresh, cambia/recupera contraseña, confirma email, tiene tablas `AspNet*`/Identity, ni `AuthService`/`UserRegistrationService`/`SignInManager`, ni rate limiting de `/api/auth/*`, ni `AdminSeed`, ni refresh tokens. **Solo VALIDA los JWT que firma Harmony** (HS256, clave simétrica **compartida** `Jwt:Key`; `Issuer`=`MRC.Agendia`, `Audience`=`MRC.Agendia.Clients`) y lee la identidad de los claims (`sub`→`NameIdentifier`, `role`→`Role`, con `MapInboundClaims=true`). El aprovisionamiento lo hace Harmony llamando a `POST /api/Business|Employee|Client` con el `sub` del usuario (los `Update` NUNCA aceptan `OwnerUserId`/`UserId`/`BusinessId`). Contrato completo en **`docs/harmony-token-contract.md`**. **⚠️ SUPERADO — histórico:** todo lo que las notas y secciones de MÁS ABAJO describen sobre auth de usuario (Identity, `AuthService`, login/register/refresh/logout, reset/forgot/change password, confirm-email, rate limiting de auth, `SmtpEmailSender`/confirmación, `AdminSeed`, refresh tokens con rotación/hasheados de #195, endurecimientos SEC-01/02/05, IDN-02) **ya NO existe en el código**; se conserva abajo solo como registro histórico. Refactor en `cf9c899` + merge #231.
> - **Auth máquina-a-máquina (M2M / client-credentials) — FEATURE, PR #233 MERGEADA a master.** Nuevo `POST /api/auth/service-token` (público, `[AllowAnonymous]`): body `{clientId, clientSecret}` → `{accessToken, expiresAt (UTC), tokenType:"Bearer"}`, **SIN refresh token** (el servicio vuelve a pedir token con su secreto al caducar). `401 INVALID_SERVICE_CREDENTIALS` (mensaje **uniforme**: clientId desconocido / secreto incorrecto / cliente deshabilitado no se distinguen); `400` si faltan campos. Agendia **firma** el token con la **misma** `Jwt:Key`/Issuer/Audience que valida (posible por ser clave simétrica compartida con Harmony; `JwtServiceTokenIssuer` replica el patrón de `TestTokenFactory`), claims cortos `sub`=`client_id`=clientId + `token_use=service` + `role`. Clientes de confianza por **configuración** (`ServiceClients[]` + `ServiceAuth:TokenLifetimeMinutes`, default 15), **SIN migración**; secreto **hasheado PBKDF2-HMAC-SHA256 con sal** (`ServiceClientSecretHasher`, formato `pbkdf2-sha256$iter$salt$hash`), verificación en **tiempo constante** (`FixedTimeEquals`). **Autorización = opción A:** el token lleva rol **`Admin`** (configurable por cliente) → acceso **transversal a todos los negocios** reutilizando la autorización actual (`CurrentBusinessScope` no restringe a Admin; `ResourceAuthorizationService` le da bypass). La opción B (rol `Service` dedicado) se descartó por tocar los ~10 bypass de autorización y todas las `RolePolicies` del flujo de usuario (fuera de alcance romperlo); el `token_use=service` deja la puerta abierta a acotarlo luego. Cada intento **auditado** (`SERVICE_TOKEN_ISSUED`/`SERVICE_TOKEN_DENIED` con el `client_id`). Capas: `Application/ServiceAuth/` (`AuthenticateServiceCommand`+handler+validador, DTOs, interfaces `IServiceClientAuthenticator`/`IServiceTokenIssuer`), `Infrastructure/ServiceAuth/` (impls + hasher + options), `Api/Controllers/AuthController`, y arm nuevo de **401** en `ExceptionHandlingMiddleware` (el único que emite Agendia; un Bearer ausente/inválido lo sigue rechazando el middleware de JWT antes). **Contrato para el servicio consumidor en `docs/service-auth-contract.md`.** ⏳ **Pendiente OPERATIVO (no código):** entregar el `clientSecret` por canal seguro y generar su hash (`ServiceClientSecretHasher.Hash`) para config/user-secrets/env del consumidor.
> - **Auditoría + limpieza (bug/refactor → automático).** El código estaba **limpio** (sin código muerto: repos/excepciones/autorización todo usado; sin sync-over-async, catch vacíos ni async void; invariantes cross-tenant intactos). **NO se hizo refactor "por refactor".** **Bug real arreglado (LOW):** los validadores de rango acotaban el **tamaño** (≤366 días) pero no el **valor absoluto** de la fecha → `DateOnly.AddDays` desbordaba en `MaxValue` (año 9999) → **HTTP 500 en vez de 400**. Nuevo `Domain/Constants/SchedulingLimits` (`MinDate`=2000-01-01, `MaxDate`=2100-12-31, alineado con el cap de `Year` de la generación) aplicado en los **5 validadores de rango**: stats, `CreateAppointmentSeries`, `GenerateScheduleTemplateInput`, `VacationPeriod`, `GetCalendar`.
> - **Tests nuevos (+32, deuda conocida cerrada):** `MappingProfilesTests` (5 — invariantes de mapeo cross-tenant en los Update de Business/Service/Employee/Client, regresión directa de #91/#125), `Application/CrudHandlers/{Business,Service,Client,Employee}HandlersTests` (24 — los 26 handlers CRUD que solo tenían cobertura de integración; fijan el contrato **autorizar-antes-de-delegar** con el id correcto + las ramas de `GetAllEmployees`: Admin ve todo, Owner solo lo suyo, cualquier otro rol 403), y `DateRangeBoundsValidatorTests` (3 — regresión del fix de desbordamiento). **Gotcha:** los tests de handlers van en namespace/carpeta `Application.CrudHandlers` (NO `Application.Business` etc.) porque un segmento de namespace `Business` colisiona con el tipo `Business` que usan otros tests (`CS0118`).
> - **Estado: 328 tests verdes** (247 unit + 81 integration), build 0/0. **Todo MERGEADO a master.** **Abierto conocido:** #185 (envío push real FCM, necesita proyecto Firebase). Nota honesta: `docs/tasks/agendia-m2m-auth.md` que citaba el owner no existía; se trabajó desde la issue #232.

> 📌 **Última actualización: 2026-05-30 (8ª sesión).** **Refactor transversal de legibilidad, SIN cambios de funcionalidad (#217, PR único de 3 commits, pendiente de review humano).** 274 tests verdes; build 0/0. Tres frentes:
> - **Reorganización por caso de uso (vertical slices, commit `db6fa50`):** los ficheros de `Application/*/Commands` y `*/Queries` (antes decenas planos) se agruparon en subcarpetas por caso de uso, con el `namespace` siguiendo a la carpeta. Ejemplos: `Auth/Commands/{Registration,Login,Logout,ChangePassword,ForgotPassword,ResetPassword,ConfirmEmail,RefreshToken}/`; `Appointments/Commands/{Crud,Series,Delay}/`; `Schedules/Commands/{Templates,Overrides,Generation,Slots}/` (`Slots/` = validators de slot/fecha COMPARTIDos por templates/overrides/generation); CRUD de Business/Clients/Employees/Services en `{Create,Update,Delete,Restore}/` y `Queries/{GetAll,GetById}/`. Las `Queries` de un solo caso de uso (Auth, Waitlist, Statistics, Availability, Auditing) se dejaron PLANAS. **Seguro porque el wiring es por ENSAMBLADO** (`RegisterServicesFromAssembly`/`AddAutoMapper(...Assembly)`/`AddValidatorsFromAssembly`), no por namespace; solo hubo que arreglar los `using` de controllers/DI/tests.
> - **Documentación XML del núcleo, en INGLÉS (commit `722906b`):** todas las **interfaces** (Application/Domain/Infra) con `<summary>`+`<param>`+`<returns>`; sus **implementaciones** con `/// <inheritdoc />`; **endpoints de controllers** con `<summary>` en inglés (traducidos del español). Se tradujeron al inglés los comentarios `//` sueltos en español que quedaban. Alcance elegido: **"núcleo"** (NO se documentaron records Command/Query, DTOs, handlers, validators ni profiles — se evaluó "exhaustivo" = 2554 miembros y el owner eligió núcleo). `GenerateDocumentationFile=true` en Api+Application con `NoWarn CS1591` (no forzamos docs en todo) + `SwaggerSetup.IncludeXmlComments` → **la Swagger UI ya muestra las descripciones de endpoints (en inglés)**.
> - **Formateo de firmas con 4+ parámetros (commits `9346154`/`140407a`):** **métodos Y constructores** con 4 o más parámetros van **multilínea paren-aligned** (estilo de la foto del owner): el **primer parámetro pegado al `(`** y los demás **alineados bajo él** (a la columna siguiente al `(`); `)` pegado al último parámetro. Es lo que produce/respeta Visual Studio y `dotnet format` (verificado estable). **Los records (DTOs/Commands/Queries) NO** — se quedan con salto-tras-`(` (un posicional con muchos campos se lee mejor así). **Solo whitespace** en ambos commits (verificado: contenido sin espacios idéntico al de partida). Ver la convención nueva en "Convenciones".
> - **Verificación:** los cambios de docs y de formato se comprobaron **docs-only / whitespace-only** (el diff no elimina ni modifica ninguna línea de código), build 0/0, 274 tests verdes, y **revisión adversarial multi-agente** (reorg+wiring y docs+formato) sin hallazgos salvo un `using` padre redundante (eliminado). **Convención NUEVA:** la documentación XML (incl. summaries de Swagger) va en **inglés**; los mensajes de runtime (validación, excepciones, logs) siguen en **español** (ver sección de idiomas).

> 📌 **Última actualización: 2026-05-30 (7ª sesión).** **i18n de notificaciones (feature) + backlog de hardening de la 6ª auditoría CERRADO.** 1 feature por PR (el humano mergeó) + **6 issues en automático** (implementar → revisar multi-agente adversarial → PR → `--admin`). **274 tests verdes** (188 unit + 86 integration; el placeholder `MRC.Agendia.API.Tests` se **eliminó**). 3 migraciones nuevas (aditivas): `AddBusinessDefaultLanguage`, `AddWaitlistUniqueWaitingIndex`, `AddWaitlistBusinessFk`.
> - **#208 — i18n de notificaciones por negocio (FEATURE, PR #209, MERGEADA):** las 5 plantillas de email/push se localizan según `Business.DefaultLanguage` (es/en/fr; default `es`). Recursos `.resx` (`NotificationStrings.resx`/`.en`/`.fr` en `Infrastructure/Notifications/Resources`) + accessor `NotificationText` con **cultura explícita** (los envíos son async, no hay `CurrentUICulture`). `SupportedLanguages` (Domain/Constants); `Business.DefaultLanguage` + migración `AddBusinessDefaultLanguage` (default 'es'). Expuesto en `BusinessDto`/`Create`/`Update` + `RegisterOwnerDto`; validadores. `WaitlistRepository` carga `Service.Business`. **Solo notificaciones**; los errores de API los traduce el front por `code` (decidido). Idioma **por negocio**, no por cliente. El HTML/estructura se queda en código; solo las frases van al `.resx`.
> - **#194 — Series (PR #210, CIERRA):** audit-log también cuando la op de serie solo produce omitidas (`created/moved>0 || skipped>0`); `MoveSeries` reporta `SERIES_MOVE_TARGET_COLLISION` (código de omisión distinto) cuando una ocurrencia choca con OTRA de la misma serie (señal dependiente del orden, por diseño de la issue, no doble-booking).
> - **#193 — Waitlist concurrencia (PR #211, CIERRA):** el trigger envuelve select-recheck-notify-mark en `IBookingConcurrencyGuard` (clave empleado+día; se llama tras commitear la cancelación, FUERA del guard → sin deadlock). Índice **único filtrado** `IX_WaitlistEntry_UniqueWaiting` (WHERE `Status=0`) + migración `AddWaitlistUniqueWaitingIndex` (`ClientId` al final para conservar el índice de FK; SQL Server trata NULL=NULL → dedup de "cualquier empleado"). Residual conocido: dos empleados distintos liberando a la vez una entrada "cualquier empleado" no se serializan (solo email duplicado).
> - **#196 — SMTP (PR #212, CIERRA):** `Email:Smtp:TimeoutSeconds` (default 15s) acotado de verdad con `CancellationTokenSource.CancelAfter` (`SmtpClient.Timeout` no acota el async); `EmailSetup` fail-fast si `User`/`Password` van a medias (ambos vacíos = relay anónimo con warning); `NotificationService.SendAsync`/waitlist separan **cargar / COMPONER / enviar** (un error de composición es permanente → devuelve handled y NO reintenta para siempre; el envío sigue siendo transitorio).
> - **#197 — Pulido API/persistencia (PR #213, CIERRA, 10 ítems):** `OperationCanceled`→499 sin cuerpo; saneo del `X-Correlation-Id` (cap 64 + charset seguro, anti log-forging); `:int` en rutas `{businessId}`; health SQL saltado en Testing; interceptor marca `CreatedAt/CreatedBy` `IsModified=false` (blinda updates detached); **FK `WaitlistEntry.BusinessId`** + migración `AddWaitlistBusinessFk` (sin índice nuevo: BusinessId ya lidera `IX_WaitlistEntry_Slot`); tie-break de plantilla `OrderByDescending(t => t.IsDefault)` (default gana) en las **TRES** copias (`ScheduleResolver`, `ScheduleTemplateRepository` y el **decorador de caché** `CachingScheduleTemplateRepository`); validación de **solape de franjas intra-día** (`WeeklySlotRules`) en los 3 validadores de plantilla; cap de rango `+1<=366` (GetCalendar/GetBusinessStats/CreateSeries); `RecurrenceExpander` ancla el patrón semanal a la **primera ocurrencia más temprana** (quincenal multi-día ya no desfasa cada día a su semana; single-day igual que antes).
> - **#195 — Auth/crypto (PR #214, PARCIAL — la issue SIGUE ABIERTA solo por el outbox):** **refresh tokens HASHEADOS** en BD (`RefreshTokenHasher` SHA-256 hex lowercase; el cliente recibe el claro; cubre store/lookup/`ReplacedByToken`). **SIN migración** (la columna `nvarchar(450)` unique no cambia; el hex lowercase resuelve además la colación CI del Base64). JWT `ValidAlgorithms=["HS256"]`. ConfirmEmail rechaza usuario inactivo. `Jwt:Key` valida **bytes** UTF-8 (≥32), no nº de caracteres. **NO hecho:** outbox de ForgotPassword (item 5; infra desproporcionada para un LOW de "solo durabilidad") → la issue queda abierta solo por eso. Nota deploy: refresh tokens en claro pre-cambio dejan de validar (re-login; sin datos en prod).
> - **#198 — Tests SQL real (PR #215, CIERRA):** `DatabaseConstraintsLocalDbTests` (`[SkippableFact]`, LocalDB, `EnsureCreatedAsync` → crea el esquema con sus índices) verifica que los índices únicos **rechazan duplicados** sobre **SQL Server real** (InMemory no impone constraints; por eso el 500 de #188 pasaba con la suite verde). Cubre override `(BusinessId,Date)`, waitlist filtrado y refresh token; asserta **violación de índice único** (SQL 2601/2627), no cualquier `DbUpdateException`. **DataProtection efímero** en `CustomWebApplicationFactory` (portable a CI cerrado). Placeholder `MRC.Agendia.API.Tests` ELIMINADO. Extensión opcional futura: `WebApplicationFactory` sobre SQL real para aseverar 400-vs-500 end-to-end por la API.
> - **La revisión adversarial cazó y se corrigió ANTES de mergear:** la 3ª copia del tie-break en el decorador de caché (#197), una regresión silenciosa de `RecurrenceExpander` single-day (#197), el `SmtpClient.Timeout` que no acotaba el envío async (#196), y se endureció la aserción de constraints a 2601/2627 (#198). Patrón confirmado: bug/refactor/test → automático con `--admin`; feature → PR para el humano.
> - **Entorno (gotcha):** el `dotnet-ef` global era 8.0.11 (proyecto en EF9) y había un `Microsoft.EntityFrameworkCore.Design.dll` 8.0.11 obsoleto en el `bin` del startup que rompía el scaffolding (`TypeLoadException` de `CSharpHelper`); `dotnet clean`/`restore --force` NO lo quitan. Solución: sobrescribirlo con el 9.0.0 del cache de NuGet (memoria `reference_dotnet_ef.md`).
...
- **Idioma de la documentación XML** (`<summary>`/`<param>`/`<returns>` de interfaces, servicios y **summaries de endpoints**): **Inglés** (decidido en la 8ª sesión). La Swagger UI muestra las descripciones de endpoints en inglés (`GenerateDocumentationFile` + `IncludeXmlComments`).
- **Idioma del texto de runtime visible al usuario** (mensajes de validación de FluentValidation, `exception messages`, logs de Serilog): **Español**. (Solo cambió el idioma de la *documentación*; los mensajes que ve el usuario en una respuesta de error siguen en español, y el front ramifica por `code`.)
...
- ✅ **274 tests verdes** (188 unit + 86 integration; el proyecto placeholder `MRC.Agendia.API.Tests` se eliminó en la 7ª sesión, #198). 4 `[SkippableFact]` LocalDB (concurrencia + constraints) corren si hay LocalDB, si no se omiten.
...
tests/
├── MRC.Agendia.Tests.Unit/       ← tests con xUnit + NSubstitute + EF InMemory (el placeholder API.Tests se eliminó en la 7ª sesión, #198)
...
16. **Firmas con 4+ parámetros → multilínea "paren-aligned"** (8ª sesión, estilo de la foto del owner). Los **métodos y constructores** con **4 o más parámetros** se escriben con el **primer parámetro en la misma línea, pegado al `(`**, y los **demás alineados justo debajo de él** (a la columna siguiente al `(`); el `)` va pegado al último parámetro. Es lo que genera Visual Studio al teclear y lo que `dotnet format` **respeta** (no lo revierte). Ejemplo:
    ```csharp
...
### Auditoría 6ª sesión — issues #185, #187–#198 (CERRADAS en la 7ª salvo dos)

> 7ª sesión: #194/#193/#196/#197/#198 MERGEADAS (PRs #210–#215); #195 PARCIAL (solo queda el outbox). #185/#172/#173 siguen fuera de alcance por decisión del owner.

| # | Título | Estado |
|---|---|---|
| [#185](https://github.com/MarcosRabadan/Agendia/issues/185) | Cablear proveedor real de push (FCM) en `PushSetup` | ⬜ Abierta (excluida; no verificable sin Firebase). |
| [#193](https://github.com/MarcosRabadan/Agendia/issues/193) | Waitlist: capacidad + serializar trigger + índice único de join | ✅ CERRADA (PR #211: concurrencia + índice único filtrado). |
| [#194](https://github.com/MarcosRabadan/Agendia/issues/194) | Series: audit-log en skip-only + colisión `MoveSeries` | ✅ CERRADA (PR #210). |
| [#195](https://github.com/MarcosRabadan/Agendia/issues/195) | Endurecimiento auth/crypto (bundle LOW) | 🟡 PARCIAL (PR #214: hash refresh tokens + JWT alg + ConfirmEmail + Jwt:Key bytes). Solo queda el **outbox de ForgotPassword** (item 5). |
| [#196](https://github.com/MarcosRabadan/Agendia/issues/196) | Fiabilidad notificaciones/SMTP (bundle LOW) | ✅ CERRADA (PR #212). |
| [#197](https://github.com/MarcosRabadan/Agendia/issues/197) | Pulido API/persistencia (bundle LOW) | ✅ CERRADA (PR #213; 10 ítems + FK WaitlistEntry). |
| [#198](https://github.com/MarcosRabadan/Agendia/issues/198) | Fidelidad de tests: integración contra SQL Server real | ✅ CERRADA (PR #215; LocalDB constraint tests + DataProtection efímero + placeholder eliminado). |
...
Los punch lists de auditoría (3ª y 6ª sesión) están **cerrados**; en la 7ª se cerró el backlog de hardening de la 6ª (#193–#198, PRs #210–#215) salvo el outbox de #195. Lo natural ahora son features o pasos de pre-producción, no más arreglos. Por orden:

1. **Lo que queda del backlog técnico (decisiones tuyas):** el **outbox de ForgotPassword** (#195 item 5 — durabilidad del envío de reset; hoy `Task.Run` best-effort) y, opcional, una `WebApplicationFactory` sobre **SQL real** para aseverar 400-vs-500 end-to-end por la API (#198 — extensión de los constraint tests LocalDB). Ambas son mejoras, no bugs. Push real **FCM** sigue en #185 (excluida: necesita Firebase).

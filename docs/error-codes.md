...
| `APPOINTMENT_CONFLICT` | `AppointmentConflictException` | Se supera la capacidad (`MaxConcurrentAppointments`) del empleado. También al **restaurar** una cita futura cuya franja se ocupó mientras estaba borrada (#294): se rechaza en vez de crear overbooking silencioso. |
...
| `SLOT_ON_HOLD` | `SlotOnHoldException` | Una plaza de la franja está reservada temporalmente para un cliente de la lista de espera (#268). El hold retiene **una plaza, no la franja**: solo salta cuando ya no quedan plazas libres al margen de las retenidas (#308). Un hold nominal cuenta contra su empleado; uno de "cualquier empleado", contra el total del negocio. |

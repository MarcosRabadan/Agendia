...
namespace MRC.Agendia.Tests.Integration.Schedules
{
    ...
    public class SchedulePreviewIntegrationTests : IClassFixture<CustomWebApplicationFactory>
    {
        ...
        private async Task<List<CalendarDayDto>> PreviewAsync(ProvisionedOwner owner, GenerateScheduleRequestDto request)
        {
            using var httpRequest = new HttpRequestMessage(
                HttpMethod.Post,
        ...
        // Every call provisions a brand new owner user id, so two calls yield two
        // genuinely isolated tenants with their own tokens.
